  ...
  try {
    ...
    if (!resolvedLeadId && lead_name && phone_number) {
      ...
      if (ex[0]) { resolvedLeadId = ex[0].lead_id; }
        ...
        const { rows: nl } = await query(`INSERT INTO leads (lead_name, phone_number, manager_id, project_id, lead_status, lead_source) VALUES ($1,$2,$3,$4,'Open','Recording') RETURNING lead_id`, [lead_name, phone_number, managerId, project_id || null]);
